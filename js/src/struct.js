...
function transform(data, spec, store, modify) {
  const cd = clone((data||{}))
  const fullstore = {
    ...cd,
    $DATA: cd,
    ...
    $PRINT: (...args)=>console.log('PRINT', ...args),
    ...
    $PACK: (mode, mpath, key, val, parent, path, nodes, current, store)=>{
      ...
      if('key:pre'===mode) {
        ...
        parent[key] = src.reduce((a,n,i)=>(a[ src[i][ parent[key]['`$KEY`'] ] ]={
          ...clone(entry),
        }, a),{})

        // console.log('PACK', mode, key, path, src, parent[key])
        
        current[cleankey] = src.reduce((a,n,i)=>
...
function inject(val, store, modify, key, parent, path, nodes, current) {
...
function injection(mode, key, val, parent, path, nodes, current, store) {
  // console.log('INJECTION', mode, key, val, path, 'C=', current)
  const find = (full, mpath) => {
    let found = mpath.startsWith('.') ?
...
function getpath(path, store, build) {
  if(null == path || '' === path) {
    return store
  }
